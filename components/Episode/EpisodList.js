...
const EpisodList = ({ darkMode, charId }) => {
    console.log(charId);
    const { data, loading, error } = useQuery(QUERY, { variables: { ids: [charId] } });
    if (loading) {
    ...
    const episods = data.charactersByIds[0].episode;
    return (
